    ...
    public boolean useOnBlock(ItemStack arg, PlayerEntity arg2, World arg3, int i, int j, int k, int l) {
        if (arg3.getBlockId(i, j, k) == Block.SLAB.id && arg3.getBlockMeta(i, j, k) == arg.getDamage()) {
            if (arg.count == 0) {
                ...
                if (arg3.method_201(i, j, k, Block.DOUBLE_SLAB.id, this.getPlacementMetadata(arg.getDamage()))) {
                    Block.BLOCKS[Block.DOUBLE_SLAB.id].onPlaced(arg3, i, j, k, l);
                    Block.BLOCKS[Block.DOUBLE_SLAB.id].onPlaced(arg3, i, j, k, arg2);
                    arg3.playSound((double)((float)i + 0.5F), (double)((float)j + 0.5F), (double)((float)k + 0.5F), var8.soundGroup.getSound(), (var8.soundGroup.method_1976() + 1.0F) / 2.0F, var8.soundGroup.method_1977() * 0.8F);
                    --arg.count;
                }
        ...
        if (arg.count == 0) {
            return false;
        } else if (j == 127 && Block.BLOCKS[this.id].material.method_905()) {
            return false;
        } else if (arg3.method_156(this.id, i, j, k, false, l)) {
            Block var8 = Block.BLOCKS[this.id];
            if (arg3.method_201(i, j, k, this.id, this.getPlacementMetadata(arg.getDamage()))) {
